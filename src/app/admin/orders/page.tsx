  ...
  return (
    <AdminShell>
      ...
      <form className="mb-5 grid gap-3 rounded-lg border border-black/10 bg-white p-4 sm:grid-cols-[1fr_180px_auto]">
        <input className="input" name="search" placeholder="Search" defaultValue={params.search || ""} />
        <select className="select" name="status" defaultValue={params.status || ""}>
          <option value="">All statuses</option>
          {["NEW", "UPSOLD", "CONTACTING", "CONFIRMED", "CANCELLED", "SHIPPED", "DELIVERED", "RETURNED"].map((status) => (
