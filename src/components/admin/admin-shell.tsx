      ...
      <header className="border-b border-black/10 bg-graphite-950 text-white">
        <div className="container flex min-h-16 items-center justify-between gap-4">
          <Link href="/admin" className="flex items-center gap-3 font-black">
            <RovanxLogo variant="mark" className="h-9 w-9" />
            <span>ROVANX Admin</span>
          </Link>
          <div className="flex items-center gap-3 text-sm">
            <span className="hidden text-white/65 sm:inline">{admin.email}</span>
            <form action={logoutAction}>
              <button className="btn border border-white/15 bg-white/10 px-3 py-2 text-sm">
                Logout
              </button>
            </form>
          </div>
        ...
        <nav className="grid content-start gap-1 rounded-lg border border-black/10 bg-white p-3">
          {nav.map(([label, href]) => (
            <Link
              key={label}
              href={href}
              className="rounded-md px-3 py-2 text-sm font-bold hover:bg-sand-50"
            >
              {label}
            </Link>
          ))}
