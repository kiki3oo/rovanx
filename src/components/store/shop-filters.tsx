...
import { localizedSeedText } from "@/components/store/seed-content";
  ...
  return (
    <form className="surface-card grid gap-3 p-4 sm:grid-cols-3">
      ...
      <select className="select" name="category" aria-label={t("categories")} defaultValue={category}>
        ...
        {categories.map((item) => <option key={item.id} value={item.slug}>{localizedSeedText(item.name, locale)}</option>)}
      </select>
